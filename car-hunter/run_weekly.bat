...
REM  /auto 로 부르면 무인 실행입니다 (작업 스케줄러가 이렇게 부릅니다).
REM   - 사람이 없으므로 pause 하지 않습니다
REM   - 리포트 창을 띄우지 않습니다 (폰에서 보므로)
REM   - 하루에 한 번만 돕니다. 마지막 실행 날짜를 적어 두고 비교합니다.
set AUTO=0
if /i "%~1"=="/auto" set AUTO=1
for /f "tokens=1-3 delims=-/. " %%a in ("%date%") do set TODAY=%%a-%%b-%%c
set STAMP=data\.last_auto_run
if "%AUTO%"=="1" (
  if exist "%STAMP%" (
    set /p LASTRUN=<"%STAMP%"
    if "!LASTRUN!"=="%TODAY%" (
      echo 오늘 이미 실행했습니다 ^(%TODAY%^). 건너뜁니다.
      exit /b 0
    )
  )
)
...
echo.
REM  실패하면 오늘 도장을 찍지 않습니다. 다음에 켤 때 다시 시도합니다.
if "%AUTO%"=="1" exit /b 1
pause
exit /b 1
